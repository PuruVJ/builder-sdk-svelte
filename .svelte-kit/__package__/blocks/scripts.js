...
export function run_scripts(el, inserted, run, label) {
    if (!el?.getElementsByTagName || typeof window === 'undefined')
        return;
    const scripts = el.getElementsByTagName('script');
    for (let i = 0; i < scripts.length; i++) {
        const script = scripts[i];
        if (script.src) {
            if (inserted.includes(script.src))
                continue;
            ...
            if (run.includes(script.innerText))
                continue;
            try {
