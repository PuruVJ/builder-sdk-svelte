...
const remove_protocol = (path) => path.replace(PROTOCOL_RE, '');
function with_width(uri, value) {
    if (WIDTH_PARAM_RE.test(uri))
        return uri.replace(WIDTH_PARAM_RE, '$1width=' + encodeURIComponent(value) + '$2');
    return uri + (uri.indexOf('?') !== -1 ? '&' : '?') + 'width=' + encodeURIComponent(value);
}
function shopify_url(src, size) {
    if (!src || !SHOPIFY_RE.test(src) || !size)
        return src;
    if (size === 'master')
        return remove_protocol(src);
...
const srcset_cache = new Map();
const MAX = 2000;
export function get_srcset(url) {
    if (!url)
        return url;
    ...
    if (BUILDER_RE.test(url)) {
        ...
        if (!isNaN(width_in_src))
            src_url = `${src_url} ${width_in_src}w`;
        ...
        out = SIZES.map((s) => [shopify_url(url, `${s}x${s}`), s])
            .filter(([u]) => !!u)
            .map(([u, s]) => `${u} ${s}w`)
            .concat([url])
            .join(', ');
        ...
        out = url;
    if (srcset_cache.size >= MAX)
        srcset_cache.delete(srcset_cache.keys().next().value);
    srcset_cache.set(url, out);
    return out;
}
export const is_builder_or_shopify = (url) => BUILDER_RE.test(url) || SHOPIFY_RE.test(url);
export const is_builder = (url) => BUILDER_RE.test(url);
export const to_webp = (srcset) => srcset.replace(QUERY_RE, '?format=webp&');
