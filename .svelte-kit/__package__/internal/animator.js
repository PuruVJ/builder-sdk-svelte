...
const assign = (style, styles) => {
    for (const key in styles)
        if (Object.prototype.hasOwnProperty.call(styles, key))
            style[key] = styles[key];
};
function elements_of(a) {
    const id = a.elementId || a.id || '';
    const els = Array.prototype.slice.call(document.getElementsByClassName(id));
    if (!els.length)
...
export function trigger_animation(a) {
    for (const el of elements_of(a)) {
        ...
        setTimeout(() => {
            ...
            if (a.delay)
                el.style.transitionDelay = a.delay + 's';
...
function bind_scroll_in_view(a) {
    for (const el of elements_of(a)) {
        augment(a, el);
        let triggered = false;
        let pending = false;
...
export function bind_animations(animations) {
    for (const a of animations) {
        if (a.trigger === 'pageLoad')
            trigger_animation(a);
        else if (a.trigger === 'scrollInView')
            bind_scroll_in_view(a);
    }
}
