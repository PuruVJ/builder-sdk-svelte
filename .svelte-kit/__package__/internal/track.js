...
export function get_cookie(name, can_track) {
    try {
        if (!can_track)
            return undefined;
...
function top_level_domain(host) {
    if (host === 'localhost' || host === '127.0.0.1')
        return host;
    const parts = host.split('.');
    return parts.length > 2 ? parts.slice(1).join('.') : host;
}
export function set_cookie(name, value, can_track, expires) {
    try {
        if (!can_track)
            return;
...
export function get_user_attributes() {
...
export async function track({ apiHost, type, canTrack, apiKey, metadata, ...properties }) {
    if (!apiKey) {
        console.error('[Builder.io]: Missing API key for track call. Please provide your API key.');
        return;
    }
    if (!canTrack || is_editing() || !is_browser())
        return;
    const event = {
        type,
        data: {
...
function builder_parent(target) {
    for (let el = target; el; el = el.parentElement) {
        const id = el.getAttribute('builder-id') || el.id;
        if (id && id.indexOf('builder-') === 0)
            return el;
    }
    return null;
}
function offset(event, target) {
    const r = target.getBoundingClientRect();
    return { x: round3((event.clientX - r.left) / r.width), y: round3((event.clientY - r.top) / r.height) };
}
export function interaction_properties(event) {
    ...
    return {
        ...
        metadata: {
            ...
            builderElementIndex: builder_el && builder_id
